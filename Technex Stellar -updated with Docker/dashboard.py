...
# allow runtime configuration via environment (docker compose sets API_URL)
API_URL = os.getenv('API_URL', 'http://127.0.0.1:5000/predict')

# health endpoint derived from prediction URL
HEALTH_URL = API_URL.replace('/predict', '/health')

# path to dataset may also be overridden
DATA_FILE = os.getenv('DATA_FILE', 'supernova_dataset.csv')
...
# perform health check after config, when the frontend container is
# more likely to have network access to the backend
if not backend_healthy():
    st.sidebar.error(f"⚠️ Cannot reach backend API at {HEALTH_URL}.\n"
                     "Single and batch predictions may fail until the service is running.")
...
# show prominent warning in page if API unreachable
if not backend_healthy():
    st.error("⚠️ Prediction backend is unreachable. Please start the Flask service or adjust API_URL.")
...
def prediction_form():
    ...
    with st.form(key='predict_form'):
        ...
        submit = st.form_submit_button("🚀 Verify Signal", width='stretch')
    ...
    if submit:
        with st.spinner("🔄 Analyzing signal..."):
            ...
            try:
                ...
                # raise for status will throw HTTPError with response attached
                r.raise_for_status()
                resp = r.json()
            except requests.exceptions.HTTPError as he:
                # show server response text for debugging
                msg = he.response.text if he.response is not None else str(he)
                st.error(f"❌ Prediction request failed: {msg}")
                return
...
def batch_upload_section():
    ...
    if uploaded is not None:
        ...
        if st.button("🚀 Start Batch Prediction", width='stretch'):
            results = []
